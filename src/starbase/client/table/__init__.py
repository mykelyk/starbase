...
from starbase.exceptions import InvalidArguments, ParseError, DoesNotExist, AlreadyExists
...
class Table(object):
    ...
    def _get(self, row, columns=None, timestamp=None, decode_content=True, number_of_versions=None, raw=False, \
        ...
        response = HttpRequest(connection=self.connection, url=url, decode_content=decode_content, fail_silently=fail_silently).get_response()
    ...
    def _scanner(self, batch_size=None, start_row=None, end_row=None, start_time=None, end_time=None, \
        ...
        response = HttpRequest(connection=self.connection, url=url, data=data, method=PUT, fail_silently=fail_silently).get_response()
    ...
    def _update_schema(self, columns, method=None, fail_silently=True):
        ...
        response = HttpRequest(connection=self.connection, url=url, data=data, method=method, fail_silently=fail_silently).get_response()
        return response.status_code
